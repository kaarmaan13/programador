<?
	...
	if (@$_REQUEST["accion"]=="guardar"){
		...
		$vsql = "select distinct * from mediador m, gerente g, sede s, mediadorobjetivosevento o where g.codgerencia=m.codgerencia and m.codsede = s.codsede and m.codsede = o.codsede ";
		...
		if ($num_rows>0){
			while($row=mysql_fetch_array($rs)) {
				...
				for ($i=0;$i<sizeof($busca);$i++){
					$vsql = "select altapolizaauto, altapolizadiversos, altapolizavida FROM mediadordatos where codmediador = ".$row["codmediador"];
		...
		header("Content-Disposition: attachment; filename=mediadorevento.xls"); 
