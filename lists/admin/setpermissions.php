...
if (!empty($_SESSION['privileges'])) {
  ...
  foreach ($_SESSION['privileges'] as $priv_category => $enabled) {
    switch ($priv_category) {
      ...
      case 'subscribers': if (!$enabled) $removeSections[] = 'subscribers'; break;
      case 'campaigns': if (!$enabled) $removeSections[] = 'campaigns'; break;
      case 'statistics': if (!$enabled) $removeSections[] = 'statistics'; break;
      case 'settings': if (!$enabled) $removeSections[]= 'config'; break;
    }
  }
